...
This extra dependency allows the usage of all link:https://docs.aws.amazon.com/sdk-for-java/v2/developer-guide/credentials.html[options] that the AWS default credential provider chain provides.

or alternatively the V1 api which does not support AWS SSO
...
If either AWS SDK is found in the classpath, it takes precedence over the custom AWS credentials lookup mechanisms listed above.
====
